...
describe('Upgrades Certificate and Bridge', () => {
	it('works', async () => {
		...
		const STokensCertificateV2 = await ethers.getContractFactory(
			'STokensCertificate'
		)
		...
		const sTokensBridgeUpgraded = await upgrades.upgradeProxy(
		...
		const sTokensCertificateAddress =
			await sTokensBridgeUpgraded.sTokensCertificateAddress()
		expect(sTokensCertificateAddress).to.equal(sTokensCertificate.address)

		// Check initialized constans of CertificateUpgraded
		const sTokensCertificateUpgraded = await upgrades.upgradeProxy(
			sTokensCertificate.address,
			STokensCertificateV2
		)
